...
species plotStockFlowMecanisms parallel: true { // Likely more efficient than with a 'reflex when: !nonGrazable' in the grid.
	...
	// OMDeposit
	aspect OMDeposited {
		float OMIntakeColourValue <- 2 * (255 / (1 + exp(-myPlot.cumulDepositedOM / 1500))) - 255; // TODO Empirical value 8 months
		float OMUptakeColourValue <- 2 * (255 / (1 + exp(-(myPlot.initialBiomassContent - myPlot.biomassContent) / maxCropBiomassContent))) - 255;
	...
	aspect nitrogenStock {
		float nitrogenColourValue <- 255 * (1 - 1 / (1 + exp(initialSoilNStock * hectareToCell - cellNstock))); // Pretty sigmoid with inflection at starting value.
